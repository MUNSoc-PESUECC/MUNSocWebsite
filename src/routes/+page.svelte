...
<main>
  ...
  <body class = "bg-body-slate prose prose-invert">
    ...
    <div class="card md:ml-10 md:mr-10 ml-5 mr-5 rounded-md text-center" onclick="window.location='{data.newsletters[0].path}'">
      ...
      <h3>{
        new Date(data.newsletters[0].meta.date).toLocaleDateString('default', { month: 'long', year: 'numeric' })
        }</h3>
    </div>
    </div>
  ...
  <!-- EVENTS -->
    ...
    <div class = "wrapper">
          ...
          <div class="card md:ml-10 md:mr-10 ml-5 mr-5 rounded-md text-center inline-block" onclick="window.location='{data.events[1].path}'">
